...
function Client (url, mode) {
  ...
  var urlElements = libUrl.parse(this.url, true);
  ...
  if(urlElements && typeof urlElements.auth === 'string') {
    var hash = new Buffer(urlElements.auth).toString('base64');
    this.authorization = 'Basic '+ hash;
    urlElements.auth = undefined;
    this.url = libUrl.format(urlElements);
  }
}
